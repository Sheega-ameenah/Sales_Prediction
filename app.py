...
with col1:
    Item_Visibility = st.number_input('**:gold[Item _Visibility_]**', min_value=0.00, max_value=0.40, step=0.01)

with col1:
    Item_MRP = st.number_input('**:gold[Item _MRP_]**', min_value=30.00, max_value=270.00, step=1.00)

with col2:
    Item_Fat_Content = st.selectbox('**:gold[Item _Fat Content_]**', ['Low Fat', 'Regular'])

with col2:
    Outlet_Size = st.selectbox('**:gold[Outlet _Size_]**', ['Small', 'Medium', 'High'])

with col3:
    Outlet_Location_Type = st.selectbox('**:gold[Outlet _Location Type_]**', ['Tier 1', 'Tier 2', 'Tier 3'])
...
if st.button('**Predict Sales**'):
    ...
    sales_prediction_output = f"**:green[Sales prediction is {sales_prediction}]**"
